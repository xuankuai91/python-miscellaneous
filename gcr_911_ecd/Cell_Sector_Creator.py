...
def main():
    ...
    work_directory = r"\\Hgac.net\FileShare\ArcGIS\DataSvcs\GIS\Process Automation\Project Folder\Tools and Applications\Cell Sector Tool"
    gdb = "CellSectors911.gdb"

    arcpy.Delete_management(os.path.join(work_directory, gdb))

    arcpy.CreateFileGDB_management(work_directory, gdb)

    arcpy.env.workspace = os.path.join(work_directory, gdb)
    ...
    arcpy.CreateFeatureclass_management(os.path.join(work_directory, gdb), "cell_towers_911_temp", "POINT", spatial_reference=nad_1983)
    ...
    print("            Creating feature class ...")
    arcpy.CreateFeatureclass_management(os.path.join(work_directory, gdb), "cell_sectors_911", "POLYGON", spatial_reference=nad_1983)
    ...
    cell_towers_csv = os.path.join(work_directory, "MapFlexDataFormat.csv")
    ...
    arcpy.FeatureClassToFeatureClass_conversion("cell_towers_911_temp", os.path.join(work_directory, gdb), "cell_towers_911", "NumberOfSectors = MAX_NumberofSectors")
...
if __name__ == "__main__":
    ...
    print("Starting Cell Sector Creator tool")
    print("Version 1.0")
